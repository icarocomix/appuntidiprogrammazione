...
```js
* Modello gli stati di una richiesta HTTP come Discriminated Union. Il campo * 'status' è il "discriminante": TypeScript lo usa per restringere il tipo in * ogni branch. */  type IdleState =
{
    status: 'idle'
}
;
...
* Funzione di exhaustiveness checking: se tutti i casi sono coperti, 'state' ha * tipo 'never'. Se aggiungo un nuovo stato alla Union senza aggiornare lo * switch, TypeScript genera un errore. */  function assertNever(state: never): never
{
    throw new Error('Stato non gestito: ' + JSON.stringify(state));
}
* Componente che usa il tipo corretto in ogni branch: TypeScript impedisce * accessi illegali. */  function renderRequestState<T>(state: RequestState<T>): string
{
    switch (state.status)
    {
        case 'idle': return 'In attesa di avvio...';
        case 'loading':  // TypeScript sa che state: LoadingState: posso accedere a startedAt const
        ...
        ms)`;
        case 'success':  // TypeScript sa che state: SuccessState<T>: posso accedere a data e fetchedAt
        ...
        `;
        case 'error':  // TypeScript sa che state: ErrorState: posso accedere a error e retriesLeft
        // return `Errore: $
        {
            state.error.message
        }
        ...
        `;
        default:  // Exhaustiveness check: se tutti i casi sono coperti, state è never return
        // assertNever(state);
    ...
    function renderRequestStateV2<T>(state: RequestStateV2<T>): string
    {
        ...
        function processPayment(payment: PaymentState): void
        {
            switch (payment.status)
            {
                ...
                `);
                break;
                case 'failed': const action = payment.retryable ? 'Riprovo' : 'Terminato';
                console.log(`Fallito: $
                {
                    payment.reason
                }
