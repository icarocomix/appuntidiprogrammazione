...
;
        ...
        -- Disabilito temporaneamente per forzare l'uso di
        SP-GiST vs GiST
...
CREATE INDEX idx_streets_prefix
    ON addresses USING SPGIST (street text_ops)
;

SELECT street
    FROM addresses
    WHERE street ^@ 'Via Gari'
;
...
SELECT street, city
    ...
    ORDER BY location <->
        point(:lon, :lat)
    LIMIT :k """, nativeQuery = true) List<AddressRow>
