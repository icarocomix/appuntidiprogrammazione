...
```java
* Le funzioni variadiche in C (quelle con ...) richiedono un FunctionDescriptor * specializzato per ogni combinazione di argomenti che intendiamo usare. Non * possiamo creare un handle "generico": ogni combinazione di tipi deve avere il * suo handle dedicato. */  @Component public class SystemFunctionCaller
{
...
");
        ...
        public void configureNetworkInterface(int socketFd, NetworkConfig config) throws Throwable
        {
            try (Arena arena = Arena.ofConfined())
            {
