...
```js
* Implemento un cluster manager robusto con auto-restart dei worker e graceful * shutdown. */  const cluster = require('cluster');
const os = require('os');
...
if (cluster.isPrimary)
{
    ...
    // cluster.on('exit', (worker, code, signal) =>
    {
        ...
        * Per applicazioni containerizzate (Docker/K8s), il clustering non è sempre la * scelta giusta: preferisco più container con un worker ciascuno per avere * isolamento, rolling deploy e scaling indipendente. Il cluster è ottimale per * VM bare-metal o quando non si vuole overhead Docker. */
```
