...
* Implemento un motore di elaborazione segnali con layout SoA per massimizzare * il prefetching: */  @Component public class SignalProcessor
{
    ...
    // frequencies; private int size; public void loadSignal(SignalData data)
    {
        ...
        // float[] contiguo }
        * Questo loop ha alta località: scansiona tre array contigui in parallelo. La * CPU prefetcha automaticamente i blocchi successivi di ogni array. */  public float[] computeEnvelope()
        {
            ...
            * Per operazioni su matrici 2D, accedo sempre per riga (row-major) perché Java * memorizza gli array per riga: */  public void processMatrix(float[][] matrix, int rows, int cols)
            {
