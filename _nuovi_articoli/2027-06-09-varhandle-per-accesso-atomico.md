...
```java
* Ottengo i VarHandle per i campi che devono essere modificati atomicamente. * Come per MethodHandle, la risoluzione avviene una sola volta in un blocco * static: il costo è allo startup, non a runtime. */  public class LockFreeStack<T>
{
    ...
    static
    {
        try
        {
            ...
            HEAD_VH = lookup.findVarHandle(LockFreeStack.class, "head", Node.class);
    ...
    * Push lock-free: uso CAS per aggiornare head atomicamente. Se un altro thread * ha modificato head nel frattempo, il CAS fallisce e riprovo. */  public void push(T value)
    {
        ...
        do
        {
            ...
            * VarHandle supporta anche le memory fence esplicite per controllo fine * dell'ordinamento della memoria: */
