...
```sql
        ...
        * l'input è costante. */
CREATE
        OR REPLACE FUNCTION convert_currency(amount numeric, from_cur text,
        to_cur text) RETURNS numeric
        AS $$
SELECT amount * rate
    FROM exchange_rates
...
CREATE
        OR REPLACE FUNCTION clean_string(input text) RETURNS text
        AS $$
SELECT
        trim(lower(input))
;
...
CREATE INDEX idx_users_clean_username
    ON users (clean_string(username))
;

        * La query userà l'indice solo se la funzione nell'indice e nella
    WHERE sono
        * identiche: */
SELECT *
    FROM users
    WHERE clean_string(username) = 'admin'
;
