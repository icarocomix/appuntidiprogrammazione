...
CREATE INDEX CONCURRENTLY idx_app_logs_gin
    ON app_logs USING GIN (payload
        jsonb_path_ops)
WITH (fastupdate =
    on, gin_pending_list_limit = 16384)
;

        -- 16MB
        /* Modifico il limite sulla tabella esistente senza ricreare l'indice: */
ALTER TABLE app_logs
    SET (gin_pending_list_limit = 16384)
;
...
;
        ...
        * Confronto il comportamento di scrittura con fastupdate
    on vs off su un batch
        * di 10.000 inserimenti: */
        -- Con fastupdate =
    on: ogni INSERT scrive solo nella lista pendente (veloce)
...
ALTER INDEX idx_app_logs_gin
    SET (fastupdate = off)
;

        -- Eseguo il bulk insert
        COPY app_logs
    FROM '/data/logs_import.csv'
WITH (FORMAT csv)
;
