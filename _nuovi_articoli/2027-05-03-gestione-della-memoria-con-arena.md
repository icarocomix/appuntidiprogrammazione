...
```java
* Esistono quattro tipi di Arena con semantiche diverse: scelgo in base al ciclo * di vita e al threading. */   // Arena.ofConfined() -> accesso da un solo thread, liberazione deterministca
// (try-with-resources)
...
{
    public byte[] applyFilter(byte[] inputImage, FilterParams params) throws Throwable
    {
        ...
        @Service public class ParallelProcessingService
        {
            public void processChunks(byte[] data, int chunkCount) throws Exception
            {
                ...
                @Test public void verifyArenaCleanup() throws Exception
                {
                    ...
                    assertTrue(leakBytes < 1024, "Possibile
