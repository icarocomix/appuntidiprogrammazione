...
<!-- <html xmlns:sec="http:
// www.thymeleaf.org/extras/spring-security">
-->
<!-- CASO 1: Visibilità basata
// su ruolo. Il div NON viene nemmeno generato per gli utenti non-admin: il
// browser non riceve l'HTML, non solo non lo vede. -->
<div
...
<!-- CASO 2:
Espressioni SpEL di sicurezza complesse. -->
<!-- Visibile solo a MANAGER o
ADMIN nella stessa organizzazione dell'utente corrente -->
<div
...
<!-- CASO 3: Accesso basato su permessi granulari
(non solo ruoli). -->
<button sec:authorize="hasAuthority('product:delete')"
th:attr="data-id=$
{product.id}
" class="btn btn-danger" onclick="deleteProduct(this.dataset.id)">
    Elimina
</button>
...
sec:authorize="isAuthenticated()">
    ...
    <!-- Accedo ai
dettagli dell'utente autenticato -->
    <span
...
<form sec:authorize="isAuthenticated()" th:action="@
{/logout}
" method="post">
...
">
        ...
        <button sec:authorize="hasRole('ADMIN')" th:attr="data-userid=$
{user.id}
