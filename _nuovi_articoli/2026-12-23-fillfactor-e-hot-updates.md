...
```sql
        /* Imposto il fillfactor al 90% per lasciare spazio agli HOT
update */
...
;
        ...
        * Verifico il rapporto tra
update normali e HOT
update: più n_tup_hot_upd si
        * avvicina a n_tup_upd, meglio è */
SELECT relname, n_tup_upd, n_tup_hot_upd, round(n_tup_hot_upd::numeric /
        NULLIF(n_tup_upd, 0) * 100, 2)
        AS hot_pct
...
;
        ...
        * gli HOT
update sono impossibili su colonne con indice */
...
;
        ...
        * Esempio di
update che genera HOT: aggiorno solo 'valore', colonna non
        * indicizzata */
UPDATE counters
    SET valore = valore + 1
    WHERE id = 42
