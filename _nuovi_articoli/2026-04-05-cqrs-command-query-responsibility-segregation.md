...
```js
* LATO COMMAND: gestisco le operazioni di scrittura con validazione e invarianti * di dominio. */  class CreatePostCommand
{
    constructor(
    {
    ...
    class PostCommandHandler
    {
        constructor(writeDb, eventBus)
        {
            ...
            async handle(command)
            {
                if (command instanceof CreatePostCommand)
                {
                    ...
                    {
                        authorId: command.authorId, content: command.content, tags: command.tags, createdAt: command.createdAt
                    }
                    );
                    // Pubblico l'evento per aggiornare le read views in modo asincrono await
                    // this.eventBus.publish('POST_CREATED',
                    {
                        postId: post.id, authorId: post.authorId, content: post.content, tags: post.tags, createdAt: post.createdAt
                    }
                    );
                    return post.id;
                }
        ...
        * LATO QUERY: gestisco le letture con modelli de-normalizzati ottimizzati per la * UI. */  class FeedQueryHandler
        {
            constructor(readDb)
            {
                ...
                )
                {
                    ...
                    * Sincronizzo i read models tramite event listener: aggiornamento asincrono * delle viste. */  class FeedProjection
                    {
                        constructor(readDb)
                        {
                            this.readDb = readDb;
                        ...
                        // async onPostCreated(event)
                        {
                            ...
                            {
                                targetUserId: followerId, postId: event.postId, authorId: event.authorId, contentPreview: event.content.substring(0, 200), tags: event.tags, createdAt: event.createdAt
                            }
                            ) ));
                        }
                    }
