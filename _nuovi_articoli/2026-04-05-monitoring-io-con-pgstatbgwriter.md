...
;
        ...
        -- Resetto le statistiche (solo in
        sessione di analisi)
...
SELECT checkpoints_timed, -- Checkpoint pianificati (normale) checkpoints_req,
        ...
        AS avg_write_ms,
        checkpoint_sync_time / nullif(checkpoints_timed + checkpoints_req, 0)
        AS
        avg_sync_ms
    FROM pg_stat_bgwriter
;
...
SELECT sum(heap_blks_hit)
        AS cache_hits, sum(heap_blks_read)
        AS disk_reads,
...
;
        ...
        return ((Number) row.get("buffers_backend")).doubleValue()
;
