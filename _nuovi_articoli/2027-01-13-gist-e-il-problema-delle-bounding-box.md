...
CREATE INDEX idx_geo_polygons
    ON city_zones USING GIST (geom)
;

        CLUSTER
        city_zones USING idx_geo_polygons
;
...
;
        ...
        * sensibilmente: */
SELECT zone_name, geom <-> ST_SetSRID(ST_MakePoint(10.2035, 45.5416), 4326)
        AS
        dist
    FROM city_zones
    ORDER BY dist
