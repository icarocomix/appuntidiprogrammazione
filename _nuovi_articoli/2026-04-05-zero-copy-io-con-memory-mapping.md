...
// Mappo l'intero file in un MemorySegment: questa operazione è istantanea  // Il kernel non carica nulla finché non accediamo ai byte MemorySegment
...
private boolean lineContains(MemorySegment segment, long start, long end, String pattern)
{
    ...
    // (long i = start; i <= end - patternBytes.length; i++)
    {
        ...
        j++)
        {
            if (segment.get(ValueLayout.JAVA_BYTE, i + j) != patternBytes[j]) continue outer;
        }
        return true;
    }
    return false;
...
public void processHugeFile(Path path) throws Exception
{
    try (FileChannel channel = FileChannel.open(path, StandardOpenOption.READ))
    {
        ...
        // 512MB per chunk for (long offset = 0; offset < fileSize; offset += chunkSize)
        {
            ...
            {
                MemorySegment chunk = channel.map(FileChannel.MapMode.READ_ONLY, offset, size, arena);
                processChunk(chunk);
            }
