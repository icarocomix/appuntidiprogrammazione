...
```js
* PATTERN SBAGLIATO: aggiungo proprietà dopo la costruzione, rompendo la Hidden * Class. V8 crea una nuova Shape per ogni variante, invalidando l'Inline Cache
...
* Verifico in Node.js se un oggetto è in Dictionary Mode (la peggiore * condizione): */   // node --allow-natives-syntax app.js function checkShape(obj)
{
...
* Per processare milioni di oggetti User in un loop stretto, la stabilità della * Shape è critica. Con la Shape monomorfica, V8 ottimizza l'intero loop con
* TurboFan: */
...
* In un server Express/Node.js che riceve JSON dall'esterno, normalizzo sempre * gli oggetti prima di processarli per garantire la stabilità della Shape: */  function normalizeUserFromJson(rawJson)
{
