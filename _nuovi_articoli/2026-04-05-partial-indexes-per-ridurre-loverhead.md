...
```sql
        ...
        * la
    WHERE della query implica la stessa condizione del predicato parziale. */
...
;
        ...
        -- Es: 128 kB vs potenziali 800 MB per un indice completo su
        milioni di righe
...
CREATE INDEX idx_jobs_queue
    ON background_jobs (priority DESC, scheduled_at
        ASC)
...
SELECT id, payload
    ...
    WHERE status IN ('QUEUED', 'RETRY')
    ORDER BY priority DESC, scheduled_at ASC FOR
UPDATE SKIP LOCKED
    LIMIT 1
;
