...
```sql
        ...
        * piano di esecuzione. */
        ...
        dentro la CTE. EXPLAIN (ANALYZE, BUFFERS)
WITH monthly_stats
        AS (
SELECT
        user_id, avg(amount)
...
;
    ...
    join sul risultato "congelato". Utile quando la CTE contiene funzioni VOLATILE
        * o aggregati costosi che non devono essere replicati per ogni riga del
    join. */
...
SELECT user_id,
    ...
    WHERE created_at >= date_trunc('month', now() -
        interval '1 month')
    GROUP BY user_id )
SELECT u.name, u.email, s.avg_spend,
        s.order_count
    FROM users u
    JOIN monthly_stats s
    ON u.id = s.user_id
